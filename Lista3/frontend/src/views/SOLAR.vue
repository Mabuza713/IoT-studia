<template>
  ...
  <p>Domyślne źródło danych to
    <a href="https://data.open-power-system-data.org/time_series/2020-10-06/time_series_30min_singleindex.csv">
      LINK
    </a>
  </p>
...
// https://bip.uke.solar.pl/download/gfx/bip/pl/defaultaktualnosci/140/5/111/pozwolenia_ntc_h_2026-01-28.csv
...
async function postHTTP() {
  ...
  const data_source = document.getElementById("data_source").value === "" ? "https://data.open-power-system-data.org/time_series/2020-10-06/time_series_30min_singleindex.csv" : document.getElementById("data_source").value
  ...
  const response = fetch('http://localhost:8003/solar', {
    method: 'POST',
    headers: {
...
async function postMQTT() {
  ...
  const topic = document.getElementById("topic").value === "" ? "solar" : document.getElementById("topic").value
  const separator = document.getElementById("separator").value === "" ? "," : document.getElementById("separator").value


  const response = fetch('http://localhost:8003/solar/publish', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
...
async function getData() {
  const data_source = document.getElementById("data_source").value === "" ? "https://data.open-power-system-data.org/time_series/2020-10-06/time_series_30min_singleindex.csv" : document.getElementById("data_source").value
  const separator = document.getElementById("separator").value === "" ? "," : document.getElementById("separator").value
  return await fetch(`http://localhost:8003/solar?data_source=${data_source}&separator=${separator}`).then(response => response.json()).then(data => data)
}
...
async function getColumns() {
  ...
  const columns_temp = await fetch(`http://localhost:8003/solar/columns?data_source=${data_source}&separator=${separator}`).then(response => response.json())
