<template>
  <h1 class="title"> NEWYORK</h1>
  <h2> Źródło danych</h2>
  <p>Domyślne źródło danych to
    <a href="https://data.cityofnewyork.us/api/views/c3uy-2p5r/rows.csv?accessType=DOWNLOAD">
      LINK
    </a>
  </p>
...
// https://bip.uke.ny.pl/download/gfx/bip/pl/defaultaktualnosci/140/5/111/pozwolenia_ntc_h_2026-01-28.csv
...
async function postHTTP() {
  ...
  const data_source = document.getElementById("data_source").value === "" ? "https://data.cityofnewyork.us/api/views/c3uy-2p5r/rows.csv?accessType=DOWNLOAD" : document.getElementById("data_source").value
  ...
  const response = fetch('http://localhost:8005/new_york', {
    method: 'POST',
    headers: {
...
async function postMQTT() {
  ...
  const topic = document.getElementById("topic").value === "" ? "ny" : document.getElementById("topic").value
  const separator = document.getElementById("separator").value === "" ? "," : document.getElementById("separator").value


  const response = fetch('http://localhost:8005/new_york/publish', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
...
async function getData() {
  const data_source = document.getElementById("data_source").value === "" ? "https://data.cityofnewyork.us/api/views/c3uy-2p5r/rows.csv?accessType=DOWNLOAD" : document.getElementById("data_source").value
  const separator = document.getElementById("separator").value === "" ? "," : document.getElementById("separator").value
  return await fetch(`http://localhost:8005/new_york?data_source=${data_source}&separator=${separator}`).then(response => response.json()).then(data => data)
}
...
async function getColumns() {
  ...
  const columns_temp = await fetch(`http://localhost:8005/new_york/columns?data_source=${data_source}&separator=${separator}`).then(response => response.json())
